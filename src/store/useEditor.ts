...
const allKeys = computed(() => {
  ...
  return [...baseKeys.filter(k => keys.has(k)), ...extraKeys];
});
...
const filteredKeys = computed(() => {
  ...
  return allKeys.value.filter(k => {
    ...
    return files.value.some(f => flatData[f.id]?.[k]?.toLowerCase().includes(q));
  });
});
...
const unflatten = (flat: Record<string, string>, order: string[]) => {
  ...
  order.forEach(key => {
    ...
    curr[parts[parts.length - 1]] = flat[key];
  });
  return res;
};
